(function () {
  ...
  async function init() {
    ...
    try {
      var res = await fetch(API + '/api/trips/' + TRIP_ID + '/journal');
      if (res.ok) { var data = await res.json(); stops = (data.trip && data.trip.stops) || []; photos = data.photos || []; reviews = data.reviews || []; }
    } catch (e) {}
    if (!photos.length) {
      try {
        var albumRes = await fetch(API + '/api/trips/' + TRIP_ID + '/photos');
        if (albumRes.ok) { var ad = await albumRes.json(); photos = ad.photos || []; }
      } catch (e2) {}
    }
    if (!photos.length) { photos = getMockPhotos(); stops = getMockStops(); reviews = getMockReviews(); }
  ...
  function renderCanvas() {
    ...
    if (!cards.length) {
      ...
      el.journalSubtitle.textContent = cards.length + ' 张卡片 · ' + TEMPLATES[currentTemplate].name + '手帐';
    }
